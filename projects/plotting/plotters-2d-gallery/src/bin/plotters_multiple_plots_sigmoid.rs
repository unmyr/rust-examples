...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program_name = match args.get(0) {
        Some(arg0) => std::path::Path::new(arg0)
            ...
            .to_str()
            .unwrap(),
        None => "plotters_multiple_plots_sigmoid",
    };
    ...
    let (y_min, y_max): (f32, f32) = (0., 1.);
    let x_axis = (x_min..x_max).step(0.1);

    // Plotting a graph of the `sigmoid` function
    let color_idx = 1;
    ...
    let mut cc = ChartBuilder::on(&upper)
        ...
        .caption("Sigmoid", ("sans-serif", 24))
        .build_cartesian_2d(x_min..x_max, y_min..y_max)?;
    let label_name = "sigmoid";
    cc.configure_mesh()
        .x_labels(5)
        .y_labels(3)
        .max_light_lines(4)
