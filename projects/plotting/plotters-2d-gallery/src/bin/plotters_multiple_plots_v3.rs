...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program_name = match args.get(0) {
        Some(arg0) => std::path::Path::new(arg0)
            ...
            .to_str()
            .unwrap(),
        None => "plotters_multiple_plots_v3",
    };
    ...
    let root_area = SVGBackend::new(&image_path, (640, 800)).into_drawing_area();
    let drawing_areas = root_area.split_evenly((3, 1));
    root_area.fill(&WHITE)?;
    println!("Drawing areas len={}", drawing_areas.len());

    // Set x-axis
    let (x_min, x_max): (f32, f32) = (-std::f32::consts::PI, std::f32::consts::PI);
    let (y_min, y_max): (f32, f32) = (-1., 1.);
    let x_axis = (x_min..x_max).step(0.1);

    // Plotting a graph of sin nθ function
    for (idx, area) in drawing_areas.iter().enumerate() {
        ...
        let label_name = format!("sin {}θ", idx + 1);
        ...
        cc.draw_series(LineSeries::new(
            x_axis.values().map(|x| (x, (x * (idx + 1) as f32).sin())),
