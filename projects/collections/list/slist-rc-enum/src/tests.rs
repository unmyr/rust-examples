...
fn test_push_back() {
    let mut list: SList<u8> = Default::default();
    assert_eq!(format!("{:?}", &list).as_str(), "SList(Nil)");
    list.push_back(1);
    assert_eq!(format!("{:?}", &list).as_str(), "SList(1) -> SList(Nil)");
    list.push_back(2);
    assert_eq!(
        format!("{:?}", &list).as_str(),
...
fn test_push_front() {
    ...
    assert_eq!(format!("{:?}", &list).as_str(), "SList(Nil)");
    list.push_front(3);
    assert_eq!(format!("{:?}", &list).as_str(), "SList(3) -> SList(Nil)");
    list.push_front(2);
    assert_eq!(
        format!("{:?}", &list).as_str(),
...
fn test_pop_front() {
    ...
    list.push_back(1);
    assert_eq!("SList(1) -> SList(Nil)", format!("{:?}", &list).as_str(),);
}
